...
- 健康保険の種別として「協会けんぽ」「健康保険組合」のいずれかを選択できる。
  ...
  - **健康保険組合を選択した場合**：
    ...
    - 細かな計算ロジックについては年金機構を参照しているため組合健保で独自の細かなルールに対応できるものではない
...
- **ロールに応じて利用できる画面・機能を制御する**：
  - **管理者（admin）**：事業所設定・ユーザー管理（ロール変更）・保険料率マスタ・従業員台帳・保険料一覧・ダッシュボードなどの全機能の閲覧・編集が可能
  - **人事・総務担当者（hr）**：従業員台帳・保険料計算・月次一覧・賞与管理・手続き管理・書類管理・納付状況管理などの実務に必要な一部編集が可能。事業所設定・ユーザー管理・マスタ編集は不可
...
- **従業員ポータル招待機能**：
  ...
  - 招待リンクは1回のみ使用可能で、使用済みの場合は再招待が必要とする

### (3) 従業員台帳・在籍・資格情報・就労条件管理機能 ✅

- 従業員の基本情報（氏名、フリガナ、生年月日、住所、電話番号、メールアドレス、入社日、退職日、所属部署、郵便番号、住所カナなど）を登録・閲覧できる
- 従業員ごとに、以下の情報を保持する
...
- 短時間労働者の社会保険適用判定に関して、システムは上記の就労条件情報を保持し、判定の参考情報として提示するが、最終的な適用判断は事業所側の責任とする
...
- 資格取得日・資格喪失日に基づき、その月に社会保険料を計上するかどうかを自動判定する
- 従業員の年齢から、介護保険の第2号被保険者（40〜64歳）に該当するかを判定し、介護保険料の有無を自動で切り替える。（月初・月末はルールに基づく）
- 従業員ごとに、就業状態を示すステータスを管理する
  - ステータス：通常勤務／産前産後休業／育児休業
  - 保険料免除月（産前産後休業・育児休業）を年月単位で指定でき、該当月の保険料は計算対象外となる
  - 注：各ステータスの開始日・終了日の詳細な期間管理は未実装。ステータスと免除月の指定のみとする
...
- 健康保険・介護保険・厚生年金について、等級ごとの標準報酬月額を事業所単位のマスタとして登録・編集できる。保険料率はマスタで事業所ごとに一律管理する
- **標準報酬月額・等級の考え方（共通方針）**
  - 健康保険組合（組合健保）・全国健康保険協会（協会けんぽ）ともに、報酬月額をいくつかの金額帯に区分し、その帯ごとに「等級」と「標準報酬月額」を決定する
...
- 厚生年金保険料総額 ＝ 標準報酬月額（厚年等級） × 厚生年金保険料率（事業主＋従業員合計）（先に総額を従業員分足す）
- 従業員負担分 ＝ 厚生年金保険料総額 ÷ 2（50銭ルール適用）（このあと従業員負担分を全員分足す）
...
- **端数処理（50銭ルール）**：従業員負担額については、給与から控除する前提として50銭以下は切り捨て、50銭超は切り上げて1円とする。月次一覧・賞与計算すべてで同一ルールを適用する
- **納入告知額の端数処理**：健康保険・介護保険の合計額と厚生年金の合計額について、端数処理前の金額を保持し、納入告知額の計算時に端数処理（円未満切り捨て・浮動小数を防ぐために小数第2で四捨五入処理もはさんでいる）を適用する
- 計算結果は対象年月ごと・従業員ごとに保持し、後から参照・比較できるようにする
- 従業員ごとに「本人負担額」「会社負担額（参考値）」「合計額」を必ず保持し、個人明細画面でも会社負担分を明示的に確認できるようにする
- 対象年月に保険資格がない従業員（資格取得日が対象年月より後、または資格喪失日が対象年月より前）は、その保険種別の保険料は計算対象外となる
- 産前産後休業・育児休業の免除月が設定されている従業員については、該当月の月次保険料は計算対象外となる。賞与については賞与登録の際に警告する
...
- 一覧には、以下のような情報を列として表示する
  - 氏名、年齢
  - 標準報酬月額（健保）、標準報酬月額（厚年）
  - 健康保険・介護保険：本人負担・会社負担・合計（介護分は40〜64歳のみ表示）
  - 厚生年金：本人負担・会社負担・合計
  - 合計本人負担額・合計会社負担額
- フィルタリングに対応し、多数の従業員でも見やすく集計できる
...
### (10) 保険料シミュレーション機能 🔼不採用

**不採用理由**：（個別の社会保険料計算は端数処理の関係で事業所負担分を正確に割り出せないことがあり概算の機能でしかなく本システムには不必要と判断）、（本来の社会保険システムの趣旨と異なると判断）

### (11) ダッシュボード機能 ✅

- 事業所ごとの社会保険料について、以下のグラフを表示する：
  - 過去12ヶ月の月次保険料推移（会社負担額）：折れ線グラフで月次保険料の推移を表示する
  - 先月・今月の保険料負担（会社負担額）：棒グラフで月次保険料と賞与保険料を分けて比較表示する
  - 年度別保険料負担（会社負担額）：棒グラフで前年度と当年度の月次保険料と賞与保険料を分けて比較表示する
...
- 賞与については、協会けんぽの定義に従い、「賃金・給料・俸給・手当・賞与など、名称を問わず労働の対価として支払われるもののうち、年 3 回以下支給されるもの（性質による）」を賞与として扱うため同じ性質の賞与を4回以上支給登録することはできない
...
- **ロール別のアクセス権限**：
  ...
  - **employee（一般従業員）**：自分の従業員情報、自分の保険料明細、自分の書類、自分宛ての書類アップロード依頼の閲覧・操作が可能（マイページのみ利用）
- 従業員別保険料明細は、当該従業員本人と、権限を持つ管理者・担当者のみが閲覧できる
...
- 人事・総務担当者が CSV ファイルをアップロードすることで、複数の従業員情報（氏名・生年月日・入社日・部署・雇用区分・報酬月額　等）を一括登録できる
- 新規作成と更新の両方に対応。ID列を空欄にすると新規作成、既存の従業員IDを入力すると更新となる
- 読み込み時に基本的な形式チェック（必須項目の確認、日付形式の検証、選択肢の妥当性チェック 等）を行い、エラー行については画面上で確認できる
...
- **月次保険料一覧のCSV出力**：
  - 対象年月、氏名、年齢、標準報酬月額、健康保険・介護保険・厚生年金の本人負担・会社負担・合計額を出力する
  - 健康保険・介護保険、厚生年金、総合計それぞれのサマリー（納入告知額、従業員負担計、会社負担計）を自動計算して出力する
- **従業員台帳のCSV出力**：
  - 従業員の基本情報、就労条件、社会保険関連情報、口座情報、給与情報を出力する
  - 現在有効な標準報酬履歴（健康保険等級・標準報酬月額、厚生年金等級・標準報酬月額）も参照用として出力する
- **賞与一覧のCSV出力**：
  - 支給日、従業員情報、賞与支給額、標準賞与額、年度内累計、健康保険・介護保険・厚生年金の本人負担・会社負担・合計額を出力する
...
- 提供されているヘルプトピック：
  - **標準報酬月額・等級とは？**：標準報酬月額の定義、等級の決定方法（定時決定・随時改定）、等級の適用期間について説明する
  - **賞与として扱うべき範囲**：社会保険上の「標準賞与額」の対象となる支給の例と、含めない支給の例を説明する
...
- 事業所ごと・対象年月ごとに、健康保険・介護保険・厚生年金の会社負担分について、「納付予定額」「納付額（実際に納付した金額）」「納付日」「納付方法」「納付ステータス（未納／納付済／一部納付／納付不要）」を記録できる
- 納付予定額は、人事・総務担当者が手動で入力する。月次保険料ページと賞与保険料ページのサマリー（健康・介護保険の会社負担額と厚生年金の会社負担額）を参照し、それぞれの合計値を入力欄に入力する
- 納付予定額は健康・介護保険（会社負担）と厚生年金（会社負担）を分けて入力し、システムが合計を計算する
...
### (22) e-Gov 電子申請連携機能 🔼不採用

**不採用理由**：（指定要件が膨大であり技術的にも基幹的にも実装不可と判断）、（必要項目も非常に多く現状のシステムにはすべての項目を取り入れることができないため実装が不可能と判断）
...
- データ不足項目の検証機能を提供し、不足している項目を警告表示する（致命的項目・主要項目・任意項目に分類）
  - 検証される項目の例：
    - 致命的項目：事業所名、被保険者氏名、生年月日
    - 主要項目：資格取得日、資格喪失日、標準報酬月額
    - 任意項目：事業所記号、退職日
...
- 協会けんぽおよび厚生年金・介護保険などの全国共通マスタについて、システム全体で共有する「クラウドマスタ」として保険料率・等級表を一元管理できる
  - システム開発者がクラウドマスタ管理ページから適用開始年月別・都道府県別（健康保険の場合）にクラウドマスタを登録・更新すると、各事業所のマスタ作成時に「料率読み込み」ボタンからクラウドマスタを初期値（テンプレート値）として読み込める（事業所側での個別更新作業を最小化する）
  - 健康保険マスタは協会けんぽのみを対象とし、都道府県別に管理する。介護保険・厚生年金マスタは全国共通として管理する
  - 組合健保など事業所ごとに異なるマスタが必要な場合は、クラウドマスタを初期値として読み込んだ後も、事業所単位で上書き・調整できる構造とする（読み込まなくてももちろんよい）
  - 事業所管理者はクラウドマスタを利用せず、手動で直接マスタを設定する
  - マスタの更新日時・更新者IDを保持し、「どの時点で更新したか」を追跡できる
  - クラウドマスタ自体の更新はシステム開発者がクラウドマスタ管理ページ（隠しページ）から手動で行い、協会けんぽ等の外部サイトからの自動取得は行わない（(6) の方針を踏襲）
- 現状ではクラウドマスタの登録は協会けんぽと年金機構の過去3年分（2023年3月改定・2024年3月改定・2025年3月改定）のみ

### (27) マイナンバー管理機能 ✅

- 拡張機能として、従業員および被扶養者のマイナンバー（個人番号）を管理できる
- マイナンバーは12桁の数字として保存され、MyNumberServiceを通じて暗号化・復号の仕組みを提供（現時点では実装はプレースホルダー、暗号化は未実装）
...
**不採用理由**：（Angular全画面i18nは規模が大きく、バグリスクが高いと判断）、（PDFなどの出力などまで対応は不可能と判断）

### (29) 社会保険情報の異常値チェック・ギャップ検知機能 ✅

- 従業員台帳・資格情報・標準報酬履歴などのデータに対して、代表的な「おかしな状態」を自動チェックする
- 実装済みのチェックルール（9ルール）：
  - **ルール1**: 資格・加入フラグ不整合（加入なのに資格取得日や標準報酬履歴が未登録、または資格取得日があるのに 未加入。入社日から1ヶ月以内の猶予期間あり）
...
- 検知された異常・要確認レコードは「異常値チェック結果」画面（`/data-quality`）に一覧表示し、人事・総務担当者が内容を確認・修正できる
...
- 異常の件数を集計し、ダッシュボード上で「要確認データ：◯件」として表示し、カードクリックで`/data-quality`へ遷移。確認済みの警告は件数に含めない
...
- ダッシュボードに「今週提出期限の手続き：◯件」「期限超過の手続き：◯件」をカード形式で表示し、カードクリックで手続き一覧画面の該当フィルタビューへ遷移
- 期限切れ行の強調表示（赤色）やステータス別バッジ表示（未着手：青、準備中：水色、提出済：緑、差戻し：赤）により、重要な手続きを見落としにくくする
- 外部通知・プッシュ通知は範囲外とする

### (31) 外部給与システム連携（CSV）機能 🔼不採用

**不採用理由**：（CSVインポート・CSVエクスポートの機能と大部分が被っているため必要でないと判断）

### (32) 口座情報・給与情報管理機能 ✅追加機能
...
- テンプレートには詳細なコメント行が含まれており、以下の情報が記載されている：
  ...
  - 標準報酬関連の注意事項
- CSVインポートの初回利用時に、項目数のずれや記入漏れによるエラーが起きることを可能な限り防ぐ
...
- 対象とするのは日本国内の 健康保険・介護保険・厚生年金保険であり、協会けんぽおよび厚生年金の標準報酬月額表・保険料率を簡略化した形で扱う。実務上のすべてのケースを網羅するものではない
...
- 短時間労働者（パート・アルバイト等）の社会保険適用について、実務上は一般に次のような条件（例：週の所定労働時間 20 時間以上、月額賃金 8.8 万円以上、雇用期間 2 か月超の見込み、学生でないこと、一定規模以上の事業所 等）をもとに判断されるが、これらの最終判断は事業所側の責任とし、本システムは事業所が「社会保険加入対象」と判断した従業員のみを isInsured を通じて管理・計算する
...
- 従業員情報の変更申請・承認ワークフローは、住所・連絡先などのプロフィール項目を対象とした1段階承認の簡易機能に限定する。賃金（報酬月額）・標準報酬月額・等級・社会保険加入フラグ・入社日・退職日など、社会保険計算に直接影響する項目は申請の対象外とし、引き続き管理者・担当者が画面から直接更新する。多段階承認や柔軟なフロー定義などの高度なワークフロー機能は対象外とする
- 給与計算そのもの（所得税・住民税・手取り額計算など）は対象外とする
- 利用環境はWebブラウザ（最新の Google Chrome / Microsoft Edge）を前提とする
