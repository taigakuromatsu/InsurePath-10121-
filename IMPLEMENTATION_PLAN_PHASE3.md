...
**目標完了日**: 2025年12月10日  
**残り日数**: 約4日間（Phase3-1からPhase3-15まで完了済み、残り1機能）
...
Phase3では、**12月10日までの完成を目指し、残り1機能を実装**します（Phase3-1からPhase3-15まで完了済み。なお、(28) 多言語対応機能は不採用、(34) 年末調整機能は実装しないことになりました）。
...
**実装完了内容**:
1. ✅ **異常値チェックルールの定義（6本柱）**
   - ルール1: 被保険者フラグと資格情報・標準報酬履歴の不整合（`insured_qualification_inconsistent`）
   - ルール2: 資格期間内なのに月次保険料レコードが存在しない（`missing_premium_record`）
   - ルール3: 資格喪失日（＋退職日）と保険料計上の矛盾（`loss_retire_premium_mismatch`）
   - ルール4: 標準報酬決定・改定履歴の期間矛盾（`standard_reward_overlap`）
   - ルール5: 介護保険料の年齢不整合（`care_premium_mismatch`）
   - ルール6: 月次保険料レコードの標準報酬スナップショット欠落（`premium_snapshot_missing`）

2. ✅ **DataQualityServiceの実装**
   - `listIssues(officeId)`メソッドによる異常値の自動検知
   - 対象年月は最新月とその前月の2ヶ月を固定判定（将来パラメータ化可能な設計）
   - 入社1ヶ月以内の猶予期間対応（ルール1）
   - 介護保険の年齢判定は月末基準
   - ルール6では0も欠落扱い

3. ✅ **異常値一覧表示画面（`/data-quality`）**
...
5. ✅ **型定義の追加**
   - `DataQualityIssueType`型の定義（6種類）
   - `DataQualityIssue`インターフェースの定義（`employeeName`を含む）
...
## 📅 Phase3の実装スケジュール（Phase3-1からPhase3-15まで完了済み、残り1機能）
...
### 12月6日（土）- Day 9
- ✅ **Phase3-14**: 手続きタスクの期限別ビュー・簡易アラート機能（完了）
