...
### (8) 月次保険料一覧・会社負担集計機能（基本実装済み）

**実装状況**: 計算・保存・一覧表示は実装済み
...
1. **マスタ管理機能（(6)）の実装**
   ...
   - 協会けんぽの初期値プリセット
   - 現在は料率を手動入力しているが、マスタから自動取得できるようにする
