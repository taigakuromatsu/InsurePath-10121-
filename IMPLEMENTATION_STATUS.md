...
2. **負担構成ダッシュボード機能（(11)）の実装**
   - 月次・賞与保険料の集計ロジック
   - グラフ表示（Chart.js等のライブラリ使用）
   - 年度別・月別の推移表示
   - 本人負担・会社負担の内訳表示

### 優先度: 中

3. **セキュリティ・アクセス制御（(13)）の強化**
   - Firestoreセキュリティルールの詳細化
   - ロール別アクセス制御の実装（画面レベル）
   - 事業所間のデータ分離（ルールレベル）
   - 識別情報のマスキング表示

4. **保険料シミュレーション機能（(10)）の実装**
   - 報酬月額・等級を入力して保険料を試算
   - 計算ロジックの再利用（premium-calculator.ts）
   - 結果表示UI
