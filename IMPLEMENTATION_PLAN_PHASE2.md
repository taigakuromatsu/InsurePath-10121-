# Phase2 実装計画

**作成日**: 2025年11月  
**最終更新**: Phase2-8完了後（2025年11月）

---

## 📊 Phase1の実装完了状況

### 完了したフェーズ
- ✅ **Phase1-2**: 従業員台帳機能の資格情報・就業状態管理
- ✅ **Phase1-3**: 社会保険料自動計算機能
- ✅ **Phase1-4**: 月次保険料一覧・会社負担集計機能（基本実装）
- ✅ **Phase1-5**: 標準報酬月額・等級・保険プランマスタ管理機能
- ✅ **Phase1-6**: 賞与管理・賞与保険料計算機能
- ✅ **Phase1-7**: マイページ機能
- ✅ **Phase1-8**: 従業員とユーザーアカウントの自動紐づけ機能
- ✅ **Phase1-9**: 月次保険料一覧機能の拡張（過去月分閲覧、絞り込み、合計行）
- ✅ **Phase1-10**: 負担構成ダッシュボード機能（基本実装）
- ✅ **Phase1-11**: 保険料シミュレーション機能
- ✅ **Phase1-12**: 負担構成ダッシュボード機能の拡張（グラフ表示）
- ✅ **Phase2-1**: セキュリティ・アクセス制御の強化
- ✅ **Phase2-2**: ロール割り当てとユーザー管理の改善
- ✅ **Phase2-3**: 被扶養者管理機能の実装
- ✅ **Phase2-4**: 扶養家族管理の導線改善
- ✅ **Phase2-5**: 標準報酬決定・改定履歴管理機能の実装
- ✅ **Phase2-6**: 標準報酬履歴の自動追加（従業員フォーム連動）
- ✅ **Phase2-7**: CSVエクスポート機能の実装
- ✅ **Phase2-8**: CSVインポート機能の実装

### Phase1で実装された主要機能
1. **基本機能**
   - 事業所管理、ユーザー・ロール管理、従業員台帳管理

2. **計算・表示機能**
   - 社会保険料自動計算
   - 月次保険料一覧・集計
   - 賞与保険料計算
   - マイページ（従業員別明細）
   - 保険料シミュレーション
   - ダッシュボード（KPIカード + グラフ表示）

3. **マスタ管理**
   - 標準報酬月額・等級・保険プランマスタ管理

### 実装率
- **完全実装**: 約62%（13/21機能）
- **部分実装含む**: 約76%（16/21機能）

---

## 🎯 Phase2の実装方針

Phase2では、**本番環境に向けた必須機能**と**実務上必要な機能**を優先的に実装します。

### Phase2の目標
1. **セキュリティ強化**: 本番環境で安全に運用できるよう、アクセス制御を強化
2. **データ管理の拡充**: 被扶養者管理、標準報酬履歴管理など、実務上必要な機能を追加
3. **運用効率化**: CSVインポート・エクスポート機能により、データ管理の効率を向上

---

## 📋 Phase2の実装フェーズ

### Phase2-1: セキュリティ・アクセス制御の強化 ✅ 完了

**優先度**: 🔴 最高（本番環境に向けた必須機能）  
**推定工数**: 高（3-4週間）  
**推定期間**: 3-4週間  
**依存関係**: なし  
**実装完了日**: 2025年11月

**目的**: Firestoreセキュリティルールと画面レベルのアクセス制御を強化し、本番環境で安全に運用できるようにする

**実装内容**:
1. **Firestoreセキュリティルールの詳細化** ✅
   - ✅ 事業所IDに基づくデータ分離（`offices/{officeId}`配下のデータは、その事業所のユーザーのみアクセス可能）
   - ✅ InsurePath向けのユーティリティ関数追加（`currentUser`, `getUserRole`, `belongsToOffice`, `isAdminOrHr`, `isInsureAdmin`, `isInsureEmployee`, `isOwnEmployee`）
   - ✅ ロール別のアクセス制御（admin/hr/employee）
   - ✅ 読み取り・書き込み権限の細分化
   - ✅ 従業員情報の更新権限（admin/hrのみ）
   - ✅ 保険料計算・保存権限（admin/hrのみ）
   - ✅ 各コレクション（事業所情報、従業員情報、月次保険料、賞与保険料、マスタ情報）への詳細なアクセス制御
   - ✅ 既存の緩いルールのコメントアウト（テスト完了後に削除予定）

2. **画面レベルのアクセス制御** ✅
   - ✅ `roleGuard`の実装（`src/app/guards/role.guard.ts`）
   - ✅ ロール別のメニュー表示制御（`app.ts`で実装）
   - ✅ 機能へのアクセス制限（`app.routes.ts`に`roleGuard`を適用）
   - ✅ 権限がない場合の適切なリダイレクト（`/me`へ遷移）
   - ✅ 管理者専用機能の保護（`/offices`, `/masters`は`admin`のみ）
   - ✅ 管理者・人事担当者用機能の保護（`/dashboard`, `/employees`, `/premiums`, `/simulator`は`admin`/`hr`のみ）

3. **識別情報のマスキング表示**
   - ⚠️ 個人情報の一部マスキング（電話番号、メールアドレスなど）は未実装（将来の拡張として検討）
   - ✅ 権限に応じた表示制御（employeeロールは自分の情報のみ閲覧可能）はFirestoreルールとガードで実現

**実装されたファイル**:
- ✅ `firestore.rules`（大幅拡張、InsurePath向けルール追加）
- ✅ `src/app/guards/role.guard.ts`（新規作成）
- ✅ `src/app/app.routes.ts`（`roleGuard`追加）
- ✅ `src/app/app.ts`（サイドメニューのロール別表示制御）

**技術要件**:
- ✅ Firestoreセキュリティルールの理解と実装
- ✅ Angular Guardsの活用
- ✅ ロール管理の実装

**注意事項**:
- ✅ 既存データへの影響を最小限に抑える（既存の緩いルールはコメントアウト）
- ⚠️ セキュリティルールのテスト（Firestore Emulator Suiteでのテスト推奨）
- ⚠️ 本番環境への移行前に、ステージング環境で十分な検証を行うことを推奨

---

### Phase2-2: ロール割り当てとユーザー管理の改善 ✅ 完了

**優先度**: 🔴 最高（Phase2-1のセキュリティ実装と密接に関連）  
**推定工数**: 中（1-2週間）  
**推定期間**: 1-2週間  
**依存関係**: Phase2-1（セキュリティ強化後が望ましい）  
**実装完了日**: 2025年1月

**目的**: 
- 「ログインしただけで全員 admin になる」現状を解消し、安全で自然なロール割り当てにする
- 「誰がどの事業所の何ロールか」を管理画面から明示的にコントロールできるようにする
- Firestore ルールでのロール制御（Phase2-1）と、ロールのライフサイクル仕様をきちんと揃える

**現状の課題**:
- 初回ログイン時に `users/{uid}` が自動生成され、その `role` が常に `admin` になっている
- そのため、どのアカウントも admin として扱われてしまい、セキュリティ意図と一致していない
- `officeId` / `role` / `employeeId` の関係が、仕様として十分に整理されていない

**実装内容**:

1. **初回ログイン時のロール初期化** ✅
   - ✅ `users/{uid}` が存在しない場合は、以下の値で作成する：
     - ✅ `role: 'employee'`
     - ✅ `officeId: undefined`（Firestoreではフィールドを削除）
     - ✅ `employeeId: undefined`（Firestoreではフィールドを削除）
   - ✅ この時点では、どの事業所にも属さず、実質的に何もできないユーザーとする
   - ✅ 既存ユーザーの `role` は変更しない（マイグレーションは別フェーズで検討）

2. **新規事業所作成時の初代admin設定** ✅
   - ✅ `/office-setup` で新規に `offices/{officeId}` を作成したユーザーは、そのオフィスの「初代 admin」として扱う
   - ✅ オフィス作成完了時に、そのユーザーの `users/{uid}` を次のように更新する：
     - ✅ `officeId = 作成した officeId`
     - ✅ `role = 'admin'`
   - ✅ これにより、各オフィスに必ず少なくとも 1 人の admin が存在する状態からスタートする

3. **既存オフィスへのメンバー追加仕様の整理** ✅
   - ✅ デフォルトでは、ログイン直後のユーザーは `officeId: null, role: 'employee'` のまま
   - ✅ `/office-setup` で既存オフィスに参加する機能を実装
   - ⚠️ **admin が明示的にユーザーを自分のオフィスに参加させる** 機能は、のちの「ユーザー管理画面」で実装予定：
     - ⚠️ 任意の `users` ドキュメントに対して `officeId` を自分のオフィス ID に設定する
     - ⚠️ `role` を `'admin' | 'hr' | 'employee'` のいずれかに変更する
   - ✅ 最初に office に参加させるときの初期ロールは `employee` を基本とする（`current-user.service.ts`の`assignOffice`メソッドで実装）

4. **ロール変更の制約**
   - ✅ **他ユーザーの `role` / `officeId` を変更できるのは admin のみ**（FirestoreルールレベルでPhase2-1で実装済み）
   - ⚠️ `hr` / `employee` は、自分を含めロール・事業所の変更はできない（UIレベルでの制約は将来の拡張）
   - ⚠️ ある `officeId` に属する admin が 1 人だけの場合、その admin は自分の `role` を `hr` / `employee` に落とせないようにする
     （「最後の admin を消さない」制約。実装は別フェーズで検討してもよい）
   - ⚠️ ロール変更時の UI 文言例：
     - 「この事業所から admin がいなくなってしまうため、ロールを変更できません。」など

5. **`employee` ロールと `employeeId` の関係** ✅
   - ✅ `role = 'employee'` に設定する場合は、同じ `officeId` 配下の従業員レコードから 1 件を選び、`employeeId` に設定する運用とする
   - ✅ `current-user.service.ts`の`assignOffice`メソッドで、メールアドレス（`contactEmail`）による自動紐づけ機能を実装
   - ✅ `admin` / `hr` については `employeeId` は任意（本人も従業員であれば紐づけてもよい）

**Phase2-2で実装した範囲**:
- ✅ 初回ログイン時の `users/{uid}` 初期値の変更（`role: 'employee'`, `officeId: undefined`, `employeeId: undefined`）
- ✅ 新規オフィス作成直後に、そのユーザーを当該オフィスの admin に昇格させる処理の追加
- ✅ 既存の Firestore ルール（Phase2-1）の `getUserRole` / `belongsToOffice` / `isAdminOrHr` 等と矛盾しないように、仕様レベルで整合性を保つ
- ✅ 既存オフィスへの参加機能（`/office-setup`で実装）
- ✅ メールアドレスによる従業員レコードの自動紐づけ機能

**将来の拡張（別フェーズ）**:
- ⚠️ 本格的な「ユーザー管理画面（ユーザー一覧 + ロール/事業所変更 UI）」の実装
- ⚠️ 既存ユーザーのロールマイグレーション機能
- ⚠️ 「最後の admin を消さない」制約の実装
- ⚠️ UIレベルでのロール変更制約の実装

**実装されたファイル**:
- ✅ `src/app/services/auth.service.ts`（`ensureUserDocument`メソッドの修正：新規ユーザーは`role: 'employee'`で初期化）
- ✅ `src/app/pages/office-setup/office-setup.page.ts`（オフィス作成後のロール更新処理追加：`updateProfile({ role: 'admin' })`）
- ✅ `src/app/services/current-user.service.ts`（`assignOffice`メソッドでデフォルトロール`'employee'`を設定、メールアドレスによる従業員レコード自動検索機能を実装）

**技術要件**:
- ✅ ユーザープロファイルの初期化ロジックの変更
- ✅ オフィス作成時のロール更新処理
- ✅ 既存のFirestoreルールとの整合性確認

**注意事項**:
- ✅ Phase2-1のセキュリティルールと整合性を保つ必要がある
- ✅ 既存ユーザーへの影響を最小限に抑える（既存ユーザーの`role`は変更しない）
- ⚠️ ロール変更の制約を明確に定義する（UIレベルでの実装は将来の拡張として検討）

---

### Phase2-3: 被扶養者管理機能の実装 ✅ 完了

**優先度**: 🟡 中（実務上必要な機能）  
**推定工数**: 中（2-3週間）  
**推定期間**: 2-3週間  
**依存関係**: Phase2-1（セキュリティ強化後が望ましい）  
**実装完了日**: 2025年1月

**目的**: 従業員の被扶養者情報を管理できる機能を実装する

**実装内容**:
1. **被扶養者情報の型定義** ✅
   - ✅ `Dependent`型の定義（氏名、続柄、生年月日、資格取得日・喪失日など）
   - ✅ Firestoreコレクション構造の設計（`offices/{officeId}/employees/{employeeId}/dependents`）

2. **被扶養者登録・編集画面** ✅
   - ✅ 基本情報の入力（氏名、続柄、生年月日など）
   - ✅ 資格取得日・喪失日の管理
   - ✅ 従業員との紐づけ
   - ✅ バリデーション（続柄の妥当性、生年月日の妥当性など）

3. **被扶養者一覧表示** ✅
   - ✅ 従業員別の被扶養者一覧（従業員詳細ダイアログ内）
   - ✅ 資格取得日・喪失日の表示
   - ✅ マイページでの閲覧機能（employeeロール向け）

4. **年次見直し支援機能**（オプション）
   - ⚠️ 基準年月日時点での被扶養者抽出（将来の拡張として検討）

**実装されたファイル**:
- ✅ `src/app/types.ts`（`Dependent`型、`DependentRelationship`型追加）
- ✅ `src/app/services/dependents.service.ts`（新規作成）
- ✅ `src/app/pages/employees/dependent-form-dialog.component.ts`（新規作成）
- ✅ `src/app/pages/employees/employee-detail-dialog.component.ts`（扶養家族セクション追加）
- ✅ `src/app/pages/me/my-page.ts`（扶養家族閲覧セクション追加）
- ✅ `src/app/utils/label-utils.ts`（`getDependentRelationshipLabel`関数追加）
- ✅ `firestore.rules`（`dependents`サブコレクションのルール追加）

**技術要件**:
- ✅ Firestoreコレクション設計
- ✅ 従業員とのリレーション管理
- ✅ 資格取得・喪失日の管理ロジック
- ✅ ロール別アクセス制御（admin/hrは追加・編集・削除、employeeは閲覧のみ）

**注意事項**:
- ✅ 被扶養者の資格取得・喪失日の管理は、実務上のルールに従う
- ⚠️ 年次見直し支援機能は、将来的な拡張として検討可能

---

### Phase2-4: 扶養家族管理の導線改善 ✅ 完了

**優先度**: 🟡 中（UI/UX改善）  
**推定工数**: 低（1週間）  
**推定期間**: 1週間  
**依存関係**: Phase2-3（被扶養者管理機能実装後）  
**実装完了日**: 2025年1月

**目的**: 扶養家族管理機能への導線を改善し、admin / hr ユーザーが直感的に扶養家族を管理できるようにする

**実装内容**:
1. **従業員一覧からの導線強化** ✅
   - ✅ 従業員一覧テーブルに「扶養家族」列を追加
   - ✅ 各行に「扶養家族を管理」ボタン（アイコン + 件数表示）を追加
   - ✅ ボタンクリック時に従業員詳細ダイアログを開き、扶養家族セクションに自動スクロール

2. **従業員詳細ダイアログ内のセクションナビ** ✅
   - ✅ セクションナビ（タブ風ボタン行）の追加
   - ✅ 各セクションへのスクロール機能
   - ✅ セクションIDの付与（`id="basic"`, `id="work"`, `id="insurance"`など）
   - ✅ スクロール位置のオフセット調整（ナビの下に余白を確保）

3. **フォーカス機能の実装** ✅
   - ✅ `EmployeeDetailDialogData`に`focusSection`プロパティを追加
   - ✅ 初期表示時に指定セクションへ自動スクロール
   - ✅ セクションナビからのスクロール機能

**実装されたファイル**:
- ✅ `src/app/pages/employees/employees.page.ts`（「扶養家族」列とアクションボタン追加）
- ✅ `src/app/pages/employees/employee-detail-dialog.component.ts`（セクションナビ追加、フォーカス機能実装）

**技術要件**:
- ✅ Angular Material Dialogの活用
- ✅ `scrollIntoView`と`scrollTo`によるスクロール制御
- ✅ `ViewChild`と`QueryList`による要素参照

**注意事項**:
- ✅ Phase2-3の実装を壊さないように、最小限の変更で実装
- ✅ My Pageの扶養家族表示は変更なし（従業員本人向け導線として維持）

---

### Phase2-5: 標準報酬決定・改定履歴管理機能の実装 ✅ 完了

**優先度**: 🟡 中（実務上必要な機能）  
**推定工数**: 中（2-3週間）  
**推定期間**: 2-3週間  
**依存関係**: Phase2-1（セキュリティ強化後が望ましい）  
**実装完了日**: 2025年11月

**目的**: 従業員の標準報酬決定・改定履歴を管理できる機能を実装する

**実装内容**:
1. **履歴管理の型定義** ✅
   - ✅ `StandardRewardHistory`型の定義（従業員ID、決定年月、標準報酬月額、決定区分など）
   - ✅ `StandardRewardDecisionKind`型の定義（定時決定、随時改定、賞与支払時、資格取得時、資格喪失時、その他）
   - ✅ Firestoreコレクション構造の設計（`offices/{officeId}/employees/{employeeId}/standardRewardHistories`）

2. **履歴登録・閲覧画面** ✅
   - ✅ 履歴登録・編集ダイアログ（決定年月、適用開始年月、標準報酬月額、決定区分、メモの入力）
   - ✅ 履歴一覧表示（従業員詳細ダイアログ内のセクションとして実装、時系列順）
   - ✅ admin/hrは追加・編集・削除可能、employeeは閲覧不可（Phase2-5ではUI側でも実装しない）

3. **適用開始年月の管理** ✅
   - ✅ 決定年月と適用開始年月の両方を入力可能（運用で後から調整可能）
   - ✅ バリデーション（YYYY-MM形式）

4. **決定区分の管理** ✅
   - ✅ 決定区分の選択（定時決定、随時改定、賞与支払時、資格取得時、資格喪失時、その他）
   - ✅ 決定区分に応じた日本語ラベル表示

**実装されたファイル**:
- ✅ `src/app/types.ts`（`StandardRewardHistory`型、`StandardRewardDecisionKind`型追加）
- ✅ `src/app/services/standard-reward-history.service.ts`（新規作成）
- ✅ `src/app/pages/employees/standard-reward-history-form-dialog.component.ts`（新規作成）
- ✅ `src/app/pages/employees/employee-detail-dialog.component.ts`（標準報酬履歴セクション追加）
- ✅ `src/app/utils/label-utils.ts`（`getStandardRewardDecisionKindLabel`関数追加）
- ✅ `firestore.rules`（`standardRewardHistories`サブコレクションのルール追加）

**技術要件**:
- ✅ Firestoreコレクション設計
- ✅ 従業員とのリレーション管理
- ✅ 時系列データの管理（`orderBy('decisionYearMonth', 'desc')`）

**注意事項**:
- ✅ 標準報酬決定・改定のルールは、実務上のルールに従う
- ✅ 履歴データは、月次保険料計算時のスナップショットとしても活用可能
- ✅ Phase2-5では履歴のCRUDと閲覧のみに焦点を当て、履歴と現在値（`employees.monthlyWage`）の連動機能は将来の拡張とする

---

### Phase2-6: 標準報酬履歴の自動追加（従業員フォーム連動） ✅ 完了

**優先度**: 🟡 中（実務上必要な機能）  
**推定工数**: 低（1週間）  
**推定期間**: 1週間  
**依存関係**: Phase2-5（標準報酬履歴管理機能実装後）  
**実装完了日**: 2025年11月

**目的**: 従業員フォームで標準報酬月額を変更したときに履歴を自動追加する機能を実装する

**実装内容**:
1. **自動履歴追加ロジック** ✅
   - ✅ 従業員編集フォームで`monthlyWage`を変更した際に、自動で`StandardRewardHistory`を1行追加
   - ✅ 新規登録時は自動追加しない（初回の履歴は将来拡張か運用でカバー）
   - ✅ `monthlyWage`が変更されていない場合は履歴を追加しない
   - ✅ 従業員保存処理が成功した後に履歴を追加

2. **自動追加される履歴の内容** ✅
   - ✅ `decisionYearMonth`: 現在の年月（`YYYY-MM`形式）
   - ✅ `appliedFromYearMonth`: 現在の年月（原則`decisionYearMonth`と同じ）
   - ✅ `standardMonthlyReward`: 保存後の`monthlyWage`
   - ✅ `decisionKind`: 一律`'other'`（手動で編集可能）
   - ✅ `note`: 自動追加であることを明記（例: "従業員フォームで標準報酬月額が変更されたため自動登録"）

3. **ハイブリッド運用** ✅
   - ✅ 自動追加された履歴もPhase2-5のUIから編集・削除可能
   - ✅ 手動での履歴追加・編集・削除機能は引き続き利用可能

**実装されたファイル**:
- ✅ `src/app/pages/employees/employee-form-dialog.component.ts`（自動履歴追加ロジック追加）

**技術要件**:
- ✅ フォーム値の変更検知（`originalMonthlyWage`との比較）
- ✅ 数値型への変換（`Number()`で数値化してから比較）
- ✅ エラーハンドリング（履歴追加失敗時も従業員保存は成功）

**注意事項**:
- ✅ 既存の月次保険料計算やマイページ表示の挙動は変えない
- ✅ `employees.monthlyWage`は引き続き「現在利用中の標準報酬月額」として扱う
- ✅ テスト用の変更も履歴に残るが、不要なら削除可能（ハイブリッド運用）

---

### Phase2-7: CSVエクスポート機能の実装 ✅ 完了

**優先度**: 🟡 中（運用効率化のための機能）  
**推定工数**: 中（2-3週間）  
**推定期間**: 2-3週間  
**依存関係**: Phase2-1（セキュリティ強化後が望ましい）  
**実装完了日**: 2025年11月

**目的**: 従業員情報や保険料データの一括エクスポート機能を実装する

**実装内容**:
1. **CSVエクスポート機能** ✅
   - ✅ 従業員台帳のエクスポート（全件エクスポート、admin/hrのみ）
   - ✅ 月次保険料一覧のエクスポート（対象年月指定、フィルタ後のデータをエクスポート）
   - ✅ 賞与一覧のエクスポート（全件エクスポート）
   - ✅ CSV形式の定義（UTF-8 + BOMエンコーディング、日本語ヘッダ、CRLF改行）
   - ✅ CSVテンプレートダウンロード機能（ヘッダ行のみ、コメント行付き）

2. **共通CSVエクスポートサービス** ✅
   - ✅ `CsvExportService`の実装（`src/app/utils/csv-export.service.ts`）
   - ✅ エスケープ処理（ダブルクォート、改行、カンマの処理）
   - ✅ UTF-8 + BOMエンコーディング対応（Excelでの文字化け防止）
   - ✅ ファイル名の自動生成（タイムスタンプ付き）

3. **UI実装** ✅
   - ✅ 従業員台帳ページに「CSVエクスポート」ボタン追加
   - ✅ 月次保険料一覧ページに「CSVエクスポート」ボタン追加
   - ✅ 賞与一覧ページに「CSVエクスポート」ボタン追加
   - ✅ 権限制御（admin/hrのみ表示）
   - ✅ CSVテンプレートダウンロードボタン（従業員台帳ページとインポートダイアログ）

**実装されたファイル**:
- ✅ `src/app/utils/csv-export.service.ts`（新規作成）
- ✅ `src/app/pages/employees/employees.page.ts`（CSVエクスポートボタン追加）
- ✅ `src/app/pages/premiums/monthly/monthly-premiums.page.ts`（CSVエクスポートボタン追加）
- ✅ `src/app/pages/premiums/bonus/bonus-premiums.page.ts`（CSVエクスポートボタン追加）

**技術要件**:
- ✅ CSV形式の文字列生成（エスケープ処理、UTF-8 + BOM）
- ✅ Blob APIとダウンロード処理
- ✅ 既存のObservable/signalストリームの再利用

**注意事項**:
- ✅ CSV形式の仕様を明確に定義（Phase2-8のインポート機能と整合性を保つ）
- ✅ エクスポートとテンプレートのフォーマット統一（`CsvImportService.getEmployeeHeaderMapping()`を使用）

---

### Phase2-8: CSVインポート機能の実装 ✅ 完了

**優先度**: 🟡 中（運用効率化のための機能）  
**推定工数**: 中（2-3週間）  
**推定期間**: 2-3週間  
**依存関係**: Phase2-7（CSVエクスポート機能実装後が望ましい）  
**実装完了日**: 2025年11月

**目的**: 従業員情報の一括インポート機能を実装する

**実装内容**:
1. **CSVインポートサービス** ✅
   - ✅ `CsvImportService`の実装（`src/app/utils/csv-import.service.ts`）
   - ✅ CSVパース機能（自前実装の簡易CSVパーサー）
   - ✅ ヘッダマッピング（日本語ヘッダ → Employeeプロパティキー）
   - ✅ バリデーション機能（必須項目チェック、データ型チェック、形式チェック）
   - ✅ コメント行サポート（`#`で始まる行をスキップ）
   - ✅ 雇用形態のマッピング（日本語入力 → 内部コード変換）

2. **インポートダイアログ** ✅
   - ✅ `EmployeeImportDialogComponent`の実装（`src/app/pages/employees/employee-import-dialog.component.ts`）
   - ✅ CSVファイル選択機能
   - ✅ プレビュー表示（最大10行、エラー行のハイライト）
   - ✅ エラー行表示（行番号とエラー内容）
   - ✅ 確認ダイアログ（インポート実行前）
   - ✅ インポート結果表示（成功件数、新規作成件数、更新件数、エラー件数、エラー詳細）
   - ✅ CSVテンプレートダウンロードボタン
   - ✅ 簡易ルール説明表示

3. **従業員台帳ページの拡張** ✅
   - ✅ 「CSVインポート」ボタン追加
   - ✅ インポート完了後の一覧再読み込み
   - ✅ 成功メッセージ表示（SnackBar）

4. **エラーハンドリング** ✅
   - ✅ 形式エラーの検出と表示（パースエラー）
   - ✅ データ整合性エラーの検出と表示（バリデーションエラー）
   - ✅ 部分的なインポート成功時の処理（エラー行はスキップ、正常行のみ保存）
   - ✅ エラーメッセージの改善（利用可能な値を表示）

5. **フォーマット統一** ✅
   - ✅ エクスポートとテンプレートのフォーマット統一（`CsvImportService.getEmployeeHeaderMapping()`を使用）
   - ✅ ヘッダ定義の二重管理を解消（`headerMapping`を単一ソースとする）

**実装されたファイル**:
- ✅ `src/app/utils/csv-import.service.ts`（新規作成）
- ✅ `src/app/pages/employees/employee-import-dialog.component.ts`（新規作成）
- ✅ `src/app/pages/employees/employees.page.ts`（CSVインポートボタン追加）
- ✅ `src/app/utils/csv-export.service.ts`（テンプレート出力機能追加、エクスポートフォーマット統一）

**技術要件**:
- ✅ CSVパース（自前実装の簡易CSVパーサー、Phase2-7のエクスポートCSVのみ対応）
- ✅ バリデーション（必須項目、データ型、形式チェック）
- ✅ エラーハンドリング（部分的な成功も許容）
- ✅ `EmployeesService.save()`の再利用（Firestoreへの保存）

**注意事項**:
- ✅ CSV形式の仕様を明確に定義（Phase2-7のエクスポート機能と整合性を保つ）
- ✅ 既存レコードの更新判定（`id`フィールドのみで判定）
- ✅ コメント行サポート（テンプレートの説明行を無視）
- ✅ 雇用形態のマッピング（日本語入力の許容）

---

## 📅 Phase2の推奨実装順序

### 第1段階（必須機能）
1. **Phase2-1: セキュリティ・アクセス制御の強化** ✅ 完了
   - 本番環境に向けた必須機能
   - 他の機能の基盤となるため、最優先で実装
   - 推定期間: 3-4週間
   - **実装完了**: Firestoreセキュリティルールの詳細化、Angular側の`roleGuard`実装、ルーティング設定への適用、サイドメニューのロール別表示制御が完了

### 第2段階（セキュリティ関連の補完）
2. **Phase2-2: ロール割り当てとユーザー管理の改善** ✅ 完了
   - Phase2-1のセキュリティ実装と密接に関連
   - Phase2-1完了後、できるだけ早く実装することを推奨
   - ロールのライフサイクル仕様を整備し、セキュリティルールと整合性を保つ
   - 推定期間: 1-2週間
   - **実装完了**: 初回ログイン時のロール初期化（`role: 'employee'`）、新規オフィス作成時の初代admin設定、既存オフィスへの参加機能、メールアドレスによる従業員レコード自動紐づけ機能が実装済み

### 第3段階（実務上必要な機能）
3. **Phase2-3: 被扶養者管理機能の実装** ✅ 完了
   - 実務上必要な機能
   - Phase2-1完了後に実装
   - 推定期間: 2-3週間
   - **実装完了**: 被扶養者情報の型定義、CRUD機能、従業員詳細ダイアログ内での表示、マイページでの閲覧機能が実装済み

4. **Phase2-4: 扶養家族管理の導線改善** ✅ 完了
   - UI/UX改善機能
   - Phase2-3完了後に実装
   - 推定期間: 1週間
   - **実装完了**: 従業員一覧からの「扶養家族を管理」ボタン追加、従業員詳細ダイアログ内のセクションナビ追加、フォーカス機能の実装が完了

5. **Phase2-5: 標準報酬決定・改定履歴管理機能の実装** ✅ 完了
   - 実務上必要な機能
   - Phase2-1完了後に実装
   - 推定期間: 2-3週間
   - **実装完了**: 標準報酬履歴の型定義、CRUD機能、従業員詳細ダイアログ内での表示が実装済み

6. **Phase2-6: 標準報酬履歴の自動追加（従業員フォーム連動）** ✅ 完了
   - 実務上必要な機能
   - Phase2-5完了後に実装
   - 推定期間: 1週間
   - **実装完了**: 従業員編集フォームでの`monthlyWage`変更時に自動で履歴を追加する機能が実装済み

### 第4段階（運用効率化）
7. **Phase2-7: CSVエクスポート機能の実装** ✅ 完了
   - 運用効率化のための機能
   - Phase2-1完了後に実装可能
   - 推定期間: 2-3週間
   - **実装完了**: 従業員台帳、月次保険料、賞与保険料のCSVエクスポート機能、CSVテンプレートダウンロード機能が実装済み

8. **Phase2-8: CSVインポート機能の実装** ✅ 完了
   - 運用効率化のための機能
   - Phase2-7完了後に実装
   - 推定期間: 2-3週間
   - **実装完了**: 従業員情報の一括インポート機能、CSVパース・バリデーション機能、インポートダイアログ、エラーハンドリング、コメント行サポート、雇用形態マッピング、エクスポートとテンプレートのフォーマット統一が実装済み

---

## 🔍 各フェーズの詳細検討事項

### Phase2-1（セキュリティ）
- **ルール設計**: Firestoreセキュリティルールの複雑さを考慮
- **テスト**: セキュリティルールのテスト方法（Firebase Emulator Suiteの活用）
- **移行**: 既存データへの影響を最小限に抑える
- **パフォーマンス**: セキュリティルールによるクエリ性能への影響

### Phase2-2（ロール割り当て）
- **初期化ロジック**: 初回ログイン時のロール初期化の実装
- **オフィス作成**: 新規オフィス作成時の初代admin設定処理
- **ロール変更制約**: adminのみがロール変更可能という制約の実装
- **整合性**: Phase2-1のFirestoreルールとの整合性確認

### Phase2-3（被扶養者管理）✅ 完了
- ✅ **データモデル**: 被扶養者情報の構造設計（従業員とのリレーション）
- ✅ **資格管理**: 資格取得・喪失日の管理ロジック
- ⚠️ **年次見直し**: 年次見直し支援機能の必要性と実装範囲（将来の拡張として検討）

### Phase2-4（扶養家族管理の導線改善）✅ 完了
- ✅ **導線強化**: 従業員一覧からの「扶養家族を管理」ボタン追加
- ✅ **セクションナビ**: 従業員詳細ダイアログ内のセクションナビ実装
- ✅ **フォーカス機能**: 特定セクションへの自動スクロール機能

### Phase2-5（標準報酬履歴）✅ 完了
- ✅ **データモデル**: 履歴情報の構造設計（従業員とのリレーション）
- ✅ **決定区分**: 決定区分の種類と管理方法（`StandardRewardDecisionKind`型）
- ✅ **適用開始**: 適用開始年月の管理（手動入力、運用で後から調整可能）

### Phase2-7（CSVエクスポート）✅ 完了
- ✅ **形式定義**: CSV形式の仕様定義（UTF-8 + BOM、日本語ヘッダ、CRLF改行）
- ✅ **エクスポート対象**: 従業員台帳、月次保険料、賞与保険料
- ✅ **テンプレート機能**: CSVテンプレートダウンロード機能（ヘッダ行のみ、コメント行付き）
- ✅ **フォーマット統一**: エクスポートとテンプレートのフォーマット統一

### Phase2-8（CSVインポート）✅ 完了
- ✅ **形式定義**: CSV形式の仕様定義（Phase2-7のエクスポート形式と整合性を保つ）
- ✅ **エラーハンドリング**: インポート時のエラー処理とユーザーフィードバック（プレビュー、エラー行表示、結果表示）
- ✅ **バリデーション**: データ整合性チェック（必須項目、データ型、形式チェック、利用可能な値の表示）
- ✅ **コメント行サポート**: `#`で始まる行をスキップ
- ✅ **雇用形態マッピング**: 日本語入力（正社員、契約社員など）を内部コードに変換
- ✅ **フォーマット統一**: エクスポートとテンプレートのフォーマット統一（`CsvImportService.getEmployeeHeaderMapping()`を使用）

---

## 📌 注意事項

- **Phase2-1（セキュリティ）は最優先**: 本番環境に向けた必須機能のため、他の機能より優先して実装する
- **Phase2-2（ロール割り当て）はPhase2-1直後に実装推奨**: Phase2-1のセキュリティ実装と密接に関連するため、できるだけ早く実装することを推奨
- **段階的な実装**: Phase2-1完了後、Phase2-3とPhase2-4は並行実装可能
- **テストの重要性**: 特にPhase2-1では、セキュリティルールのテストを十分に行う
- **既存機能への影響**: Phase2-1の実装により、既存機能への影響がある可能性があるため、十分な検証を行う

---

## 🚀 Phase2以降の展望

Phase2完了後、以下の機能をPhase3以降として実装予定です：

### Phase3（2025年12月10日までに完成予定）
1. **従業員情報の最終更新者・更新日時表示機能の完成（(16)）**
   - 一覧画面での表示追加

2. **社会保険用語・ルールヘルプ機能（(17)）**
   - ヘルプコンテンツの作成
   - ヘルプアイコン・モーダルの実装

3. **従業員情報の変更申請・承認機能（(18)）**
   - 申請登録画面
   - 承認・却下機能
   - 申請一覧表示

詳細は`IMPLEMENTATION_PLAN_PHASE3.md`を参照してください。

### Phase4（将来拡張）
1. **社会保険手続き履歴・期限管理機能（(19)）**
   - 手続き履歴の型定義とCRUD機能
   - 提出期限の自動計算
   - 期限切れアラート

2. **被扶養者状況確認・年次見直し支援機能（(20)）**
   - 扶養状況確認の型定義とCRUD機能
   - 基準年月日時点での抽出機能

3. **納付状況管理機能（(21)）**
   - 納付状況の型定義とCRUD機能
   - 納付予定額・実納付額の記録
   - 納付ステータス管理

詳細は`IMPLEMENTATION_PLAN_PHASE4.md`を参照してください。

---

以上でPhase2の実装計画は完了です。各フェーズの詳細な実装指示書は、実装開始前に作成することを推奨します。

