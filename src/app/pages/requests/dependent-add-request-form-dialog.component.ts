...
export interface DependentAddRequestFormDialogData {
  officeId: string;
  employeeId: string;
}

@Component({
  selector: 'ip-dependent-add-request-form-dialog',
  standalone: true,
  imports: [
    MatDialogModule,
    ReactiveFormsModule,
  ...
  template: `
    <h1 mat-dialog-title>扶養家族追加申請</h1>
...
export class DependentAddRequestFormDialogComponent {
  ...
  constructor(
    private readonly dialogRef: MatDialogRef<DependentAddRequestFormDialogComponent>,
    @Inject(MAT_DIALOG_DATA)
    public data: DependentAddRequestFormDialogData
  ) {}
  ...
  async submit(): Promise<void> {
    ...
    const payload: DependentAddPayload = {
      name: formValue.name ?? '',
