...
- 実装済みのチェックルール（6本柱）：
  - **ルール1**: 被保険者フラグと資格情報・標準報酬履歴の不整合（isInsured = true なのに資格取得日や標準報酬履歴が未登録、または資格取得日があるのに isInsured = false。入社1ヶ月以内の猶予期間あり）
  - **ルール2**: 資格期間内なのに月次保険料レコードが存在しない（対象年月は最新月とその前月の2ヶ月を固定判定）
  - **ルール3**: 資格喪失日（＋退職日）と保険料計上の矛盾（喪失・退職後も保険料が計上されている）
  - **ルール4**: 標準報酬決定・改定履歴の適用期間矛盾（appliedFromYearMonth が重複・逆順になっている）
  - **ルール5**: 介護保険料の年齢不整合（40～64 歳に該当するのに介護保険料が発生していない、またはその逆。年齢判定は月末基準）
  - **ルール6**: 月次保険料レコードの標準報酬スナップショット欠落（等級・標準報酬月額が未設定または0）
- 検知された異常・要確認レコードは「異常値チェック結果」画面（`/data-quality`）に一覧表示し、人事・総務担当者が内容を確認・修正できる。issueTypeは日本語ラベルで表示される。
- 異常の種類ごとに件数を集計し、ダッシュボード上で「要確認レコード：◯件」としてサマリ表示し、カードクリックで`/data-quality`へ遷移できる。
- `DataQualityIssue`はFirestoreに保存せず、画面表示時に毎回計算して表示する方式。「無視」「確認済み」フラグは今回のスコープ外。


### (30) 手続きタスクの期限別ビュー・簡易アラート機能 ✅

- (19) 社会保険手続き履歴・期限管理機能と連携し、「いつまでに何の手続きを行う必要があるか」をタスク一覧として見える化する。
- 手続き一覧で、手続き種別・対象者・事由日・提出期限・ステータスをまとめて確認できる。
- 期限別ビューで未完了の手続きを絞り込み：  
  - 「今週提出期限」  
  - 「来週提出期限」  
  - 「7日以内の期限（期限が近い）」  
  - 「期限切れ（提出期限を過ぎて未完了）」  
- ダッシュボードに「今週提出期限の手続き：◯件」「期限超過の手続き：◯件」を表示し、カードクリックで詳細一覧へ遷移できる。
- 期限切れ行の強調表示やステータス別バッジ表示により、重要な手続きを見落としにくくする（外部通知・プッシュ通知は範囲外）。
